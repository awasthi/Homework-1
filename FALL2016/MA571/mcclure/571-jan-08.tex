...
\begin{solution}
  ...
  \(B_n\subset U_n\) if such an \(n\) exists; otherwise, associate to
...
\begin{solution}
  ...
  Moreover, since \(\bbR^n\) is Hausdorff and \(f(X)\subset\bbR^n\) is a
...
\begin{solution}
  ...
  in the latter case \(x\in D\). Thus, \(C\cup D\supset X\) and
  \(C\cup D=X\); it is clear that both \(C,D\subset X\) so the union
  \(C\cup D\subset X\).
...
\begin{problem}
  ...
  in \(B\). Let \(A\subset E\) be compact. Prove that for every
...
\begin{solution}
  ...
  \(A\cap p^{-1}\{b\}\subset\bigcup_{n=1}^N U_n\). Since each
