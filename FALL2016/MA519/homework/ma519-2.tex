...
\end{problem}
\begin{solution}
  Let \(A\) denote the event that the youngest girl broke a dish and \(B\)
  denote the event that somebody broke a dish. By Bayes' theorem
  \[
    P(A\mid B)=\frac{P(B\mid A)P(A)}{P(B\mid A)P(A)+P(B\mid A^\sfc)P(A^\sfc)}.
  \]
  We know that \(P(A)=3/4\) and that \(P(B\mid A^\sfc)\)
\end{solution}
\newpage
