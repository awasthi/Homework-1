...
\begin{solution}
  ...
  % Before we find the densities of \(L_s\), \(L_m\), and \(L_l\), let us
  % first find the densities of \(X\), \(Y\), and \(Z\). We can compute the
  % conditional probability of one of these densities rather quickly. This is
  % \[
  %   f_{X\,|\,w}(x)=
  %   \begin{cases}
  %     \frac{1}{w}&\text{for \(0\leq x\leq w\),}\\
  %     0&\text{otherwise.}
  %   \end{cases}
  % \]
  % Now we can find the density of \(Y\) by first finding its CDF,
  % \begin{align*}
  %   f_Y(x)
  %   &=\int_{-\infty}^\infty f_{-X\,|\,w}(x-y)f_W(y)\diff y\\
  %   &=\int_{-\infty}^\infty
  % \end{align*}
\end{solution}
