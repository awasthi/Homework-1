...
\begin{solution}
  ...
  the \(i\)\textsup{th} roll. Then,
  \[
    E(x_1+\dotsb+x_{n-2})=E(x_1)+\dotsb+E(x_{n-2})=p_1+\dotsb+p_{n-2}.
  \]
  We need to find these \(p_i\). There are \(6^n\) points in the sample
  space \(\omega\). Starting at the \(i\)\textsup{th} place, we can have
  \(6^{n-i-2},6^{n-i-3},\dotsc,6\) different successions corresponding to
  three sixes in a row, four sixes in a row, etc. Thus,
  \begin{align*}
    p_i&=\frac{6^{n-i-2}+\dotsb+6}{6^n}\\
       &=6^{-i-2}\bigl(1+\dotsb+6^{-n+i+3}\bigr)\\
       &=
  \end{align*}
\end{solution}
\newpage
...
\end{problem}
\begin{solution}
  Let $A_i$ be the event that the $i$\textsup{th} class is not represented
  in the sample of size $n$. Let $A$ be the number of classes not
  represented in the sample of size $n$. Let $A_i$ also represent the
  number of times that the $i$\textsup{th} class is not represented. Then
\begin{align*}
