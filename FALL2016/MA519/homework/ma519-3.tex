...
\begin{solution}
  ...
  partition of \(\Omega\)
  \[
    A_i=\bigl\{\,\text{exactly \(i\) black balls are put into the second urn}\,\bigr\}.
  \]
  The events \(A_i\), \(1\leq i\leq 5\), are clearly mutually
  exclusive. Therefore, to find \(P(A)\), we need only find the
  probabilities \(P(B|A_i)\) and \(P(A_i)\).
\end{solution}
\newpage
