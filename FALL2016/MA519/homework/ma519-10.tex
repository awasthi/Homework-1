...
\begin{problem}[Handout 14, \# 7]
  ...
  approximation to the probability that \(\sum_{k=1}^n X_k\) is larger than
  \(\sum_{k=1}^n X_k^2\), when \(n=10,20,30\).
\end{problem}
\begin{solution}
  Suppose \(X_1,\dotsc,X_n\) are independent \(N(0,1)\) variables. We know
  that the sum \(S_2\defeq\sum_{k=1}^n X_k^2\) is a chi-square distribution
  with mean \(n\), variance \(2n\), and density
  \[
    f_{S_2}(x)=\frac{\rme^{-x/2}x^{n/2-1}}{2^{n/2}\Gamma(m/2)}.
  \]
  Moreover, by Theorem 1.46, the distribution of the sum
  \(S_1\defeq\sum_{k=1}^n X_k\) is exactly a \(N(0,n)\)
