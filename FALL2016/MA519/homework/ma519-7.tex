...
\end{problem}
\begin{solution}
  Suppose \(X\sim\Poisson(\lambda)\) and let us consider the case in which
  \(X\) is continuous and takes values on \(\bbR_{\geq 0}\). Then, the PMF
  of \(X\) has the form
  \[
    p'(x)=e^{-\lambda}\frac{\lambda^x}{\Gamma(x)}
  \]
\end{solution}
\newpage
