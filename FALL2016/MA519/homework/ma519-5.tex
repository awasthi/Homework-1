...
\begin{solution}
  ...
  Lyapunov's inequality, the mean of \(X\) exists and is finite. Moreover,
  we have \(\mu\leq 1\). By Hölder's inequality, given \(X^n\), decompose
  \(n\) modulo \(4\), then modulo \(2\) and by Hölder's inequality, the
  \(n\)\textsup{th} moment is less than or equal to \(1\).
\end{solution}
\newpage
