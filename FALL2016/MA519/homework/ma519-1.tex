...
\begin{solution}
  ...
  does not matter, the cardinality of the sample space \(\Omega\) is
  \begin{equation}
    \label{eq:1-1}
    \#\Omega=%
    \binom{10}{4}=%
  ...
  about? Let \(A\) denote the event that at least \(1\) complete pair of
  shoes is among the \(4\). Then we can split \(A\) (as a set) as the union
  of
  \[
    A_1=\bigl\{\,\text{exactly \(1\) pair is among the \(4\)}\,\bigr\}
  \]
  and
  \[
    A_1=\bigl\{\,\text{exactly \(2\) pairs are among the \(4\)}\,\bigr\}.
  \]
  We can count the number of points in \(A_2\) easily enough these are
  \begin{equation}
    \label{eq:1-2}
   \#A_2=\binom{5}{2}=\frac{5!}{2!3!}=\frac{5\cdot 4\cdot 3!}{2\cdot 3!}=5\cdot 2=10,
  \end{equation}
  since we are not taking into consideration the order in which we select
  the pair.

  To count the points in \(A_1\) we observe that there are \(5\) pairs to
  ...
  \(7-1=6\) other shoes to choose from; \ie{} there are exactly
  \begin{equation}
    \label{eq:1-3}
    5\cdot 4\cdot 6=120
  \end{equation}
  ways to choose one pair. Thus, the probability that there is at least one
  complete pair among the four is
  \[
    \#A_1=p(A)=p(A_1)+p(A_2)=\frac{120}{210}+\frac{10}{210}=\frac{130}{210}\approx
    0.619.
  \]
\end{solution}
\newpage
