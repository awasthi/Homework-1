...
\end{problem}
\begin{solution}
  For part (a), using the axioms of a measure, exactly two events occurring
  is \(AB+AC+BC-ABC\)
  \[
    p(AB+AC+BC)=p(AB)+p(AC)+p(BC)-2p(ABC).
  \]

  For part (b), exactly one event occurring is
  \[
    p(A)+p(B)+p(C)-p(AB)-p(AC)-p(BC)+p(ABC).
  \]

  For part (c), the probability that no event occurs is
  \[
    p(\Omega)-p(A)-p(B)-p(C)+p(AB)+p(AC)+p(BC)+2p(ABC).
  \]
  This is easy to see using the diagram
\end{solution}
\newpage
