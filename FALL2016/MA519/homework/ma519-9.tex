...
\begin{solution}
  ...
  \begin{align*}
    ...
    \intertext{which by substitution with \(u=x^2+1\), \(du=2x\diff x\),
    evaluates to}
    &=\lim_{x_1,x_2\to\infty}\tfrac{2}{\pi}\ln\left(\frac{x_2^2+1}{x_1^2+1}\right).
  \end{align*}
  Fix \(x_2\). Then,
  \[
    \lim_{x_1\to\infty}\tfrac{2}{\pi}\ln\left(\frac{x_2^2+1}{x_1^2+1}\right)=\infty.
  \]
  Whereas, if we fix \(x_1\),
  \[
    \lim_{x_2\to\infty}\tfrac{2}{\pi}\ln\left(\frac{x_2^2+1}{x_1^2+1}\right)=-\infty.
  \]
  We conclude that the limit of this function as \(x_1,x_2\to\infty\) does
  not exists. Thus, \(E(X)\) does not exist.

  For \(-X\), first we find the CDF:
  \begin{align*}
    F_{-X}(x)
    &=P(-X\leq x)\\
    &=1-P(X\leq x)\\
    &=1-\frac{1}{\pi}\tan^{-1}(x)-\frac{1}{2}\\
