...
\begin{solution}
  ...
  \begin{align*}
    0&=\Delta u\\
     ...
     &=\sum_{n\in\bbZ}n(n-1)a_nr^{n-2}(\cos(n\theta)+\sin(n\theta)\bigr)\\
     &\phantom{{}={}}%
       +\sum_{n\in\bbZ}na_nr^{n-2}(\cos(n\theta)+\sin(n\theta)\bigr)\\
     &\phantom{{}={}}%
       +\sum_{n\in\bbZ}na_nr^{n-2}\bigl(-\sin(n\theta)+\cos(n\theta)\bigr)\\
     &=\sum_{n\in\bbZ}
       r^{n-2}\left(n^2a_n\cos(n\theta)+n(n-1)a_n\sin(n\theta)\right).
  \end{align*}
  This implies that
  \[
    0=n^2a_n\cos(n\theta)+n(n-1)a_n\sin(n\theta).
  \]
  Hence,
  \[
    a_n=-\tfrac{n(n-1)}{n^2}a_n\tan(n\theta)
  \]
\end{solution}
\newpage
