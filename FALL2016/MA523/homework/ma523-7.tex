...
\begin{problem}
  ...
  solution of \eqref{eq:7:neumann-problem} in \(C^2(\Omega)\cap
  C^1(\bar\Omega)\) is unique up to a constant; i.e., if \(u_1\) and
  \(u_2\) are both solutions of \eqref{eq:7:neumann-problem}, then
  \(u_2=u_1+\text{const.\@}\) in \(\Omega\).
...
\begin{solution}
  ...
  \eqref{eq:7:neumann-problem}. Set \(v\defeq u_1-u_2\). Then \(v\) is
  harmonic in \(\Omega\)
\end{solution}
\newpage
