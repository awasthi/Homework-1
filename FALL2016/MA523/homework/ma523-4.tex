...
\begin{solution}
  ...
  \end{align*}
  where \(C'+C''=0\). Hence, the solution tho the PDE has the form
  \begin{equation}
    \label{eq:3:sol}
    u(x,t)
    =\frac{f(x+t)+f(x-t)}{2}+\frac{1}{t}\int^{x+t}_{x-t}g(\xi)\diff\xi.
  \end{equation}

  Let us verify that \eqref{eq:3:sol} is in fact a solution to the PDE at
  hand. Substituting \eqref{eq:3:sol} for \(u\) in the PDE, we have
  \begin{align*}
    u
  \end{align*}
\end{solution}
