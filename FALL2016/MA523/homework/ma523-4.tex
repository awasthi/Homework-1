...
\begin{solution}
  ...
  one-dimensional wave equation. Then, by the boundary and initial
  conditions prescribed above, we obtain the following relations on \(G\)
  and \(F\),
  \begin{align*}
    u(x,0)
    &=F(x)+G(x)\\
  ...
  \begin{equation}
    ...
    \right.
  \end{equation}
  After a bit of calculation using equations (4.12) from [J], we have
  \[
    \int_{x-t}^{x+t}g(s)\diff s=\alpha\bigl(f(x+t)+f(x-t)\bigr).
  \]
\end{solution}
\newpage
