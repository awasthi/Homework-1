...
\begin{problem}
  ...
  \[
    ...
    \right.
  \]
  exists in \(\bfR\times[0,t)\); i.e., the first time \(t=t_0\) when the
  shock develops.
\end{problem}
\begin{solution*}
...
\begin{problem}
  ...
  \item Is it true that every smooth simple closed curve \(\Gamma\) in
    \(\bfR^2\) has at least one point at which it is characteristic with
    respect to \(L\)?
  \end{enumerate}
\end{problem}
\begin{solution*}
...
\begin{problem}
  ...
  \item Find the points on the line
    \(\Gamma\defeq\{\,(x,y)\in\bfR^2:x+y=1\,\}\) at which \(\Gamma\) is
...
\begin{problem}
  ...
  \(\{\,x\in\bfR^n:|x|<1,x_n>0\,\}\). Assume \(u\in C(\bar B_1^+)\) is
...
\begin{problem}
  ...
  \(B_1\subset\bfR^n\). What can you conclude about \(u\) and \(v\) if
  \begin{enumerate}[label=(\alph*),noitemsep]
...
\begin{problem}
  ...
  \(\bfR^n\) and \(f\in C_0^\infty(\bfR^n)\). Then the convolution
  \[
    u(x)\defeq(\Phi* f)(x)=\int_{\bfR^n}\Phi(x-y)f(y)\diff y
  \]
  is a solution to the Poisson equation \(-\Delta u=f\) in \(\bfR^n\). Show
