...
\begin{problem}
  Consider the first order equation in \(\bfR^2\)
  \[
  ...
  \[
    \left\{
      \begin{aligned}
        ...
        u(x',0)&=g(x')&&\text{for \(x'\in\bfR^{n-1}\),}
      \end{aligned}
    \right.
  \]
  where \(g\) is a continuous function with compact support in
  \(\bfR^{n-1}\). Here \(n\geq 2\). Prove that
  \[
    u(x)\To 0,\qquad%
    \text{as } |x|\To \infty,
  \]
...
\begin{problem}
  ...
  \[
    \Delta u-u_t=0\qquad\text{in \(\bfR\times(0,\infty)\),}
  \]
  with the initial conditions \(u(x,0)=g(x)\), where \(g\) is a bounded
  continuous function on \(\bfR\) satisfying the Hölder condition
  \[
    |g(x)-g(y)|\leq M|x-y|^\alpha,\qquad x,y\in\bfR
  \]
  with a constant \(\alpha\in (0, 1]\). Show that
  \[
    \begin{aligned}
      |u(x,t)-u(y,t)|&\leq M|x-y|^\alpha,&&x,y\in\bfR,t>0,\\
      |u(x,t)-u(x,s)|&\leq C_\alpha M|t-s|^{\alpha/2},&&x\in\bfR,t,s>0.
    \end{aligned}
  \]
...
\begin{problem}
  ...
  \[
    \left\{
    \begin{aligned}
      u_{tt}-\Delta u&=|x|^k&&&&\text{in \(\bfR^n\times(0,\infty)\),}\\
      u&=0,&u_t&=0&&\text{on \(\bfR^n\times\{0\}\)}.
    \end{aligned}
    \right.
  \]
