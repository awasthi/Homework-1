...
\begin{problem}[Wheeden \& Zygmund Ch.\@ 3, Ex.\@ 5]
  ...
  \(\delta 3^{-k}\), \(0<\delta<1\). Show that the resulting set is
  perfect, has measure \(1-\delta\), and contains no interval.
\end{problem}
\begin{solution}
  ...
  \[
    ...
    \end{aligned}
  \]
  whose union we call \(E_1\); this marks the first step in the
  construction of this Cantor-like set. Next, we remove the set
  \[
  ...
  yielding \(E_n\) which is the union of \(2^n\) intervals \(E_{n,i}\) of
  length \(2^{-n}-\delta 2^{-n}\sum_{i=1}^n2^{i-1}3^{-i}\). Let \(E\) be
  the intersection \(\bigcap_{i=1}^\infty E_i\). This concludes our
  construction.

  Next we show that \(E\) is perfect, has measure \(1-\delta\) and contains
  no interval.
