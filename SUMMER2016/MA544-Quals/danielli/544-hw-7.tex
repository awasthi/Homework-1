...
\end{problem}
\begin{solution}
  For a fixed point $x_0\in\bbR^n$, for any $m<g(x_0)$, it suffices to show
  that there exists $\delta>0$ such that for every $x\in B(x_0,\delta)$,
  $g(x)>m$. Since $g(x_0)$ is the supremum of $f(x)$ over all
  $x\in B(x_0)$, given $\varepsilon>0$ such that $g(x_0)-\varepsilon>m$,
  there exists $x_1\in B(x_0)$ such that $f(x_1)>g(x_0)-\varepsilon$
\end{solution}
