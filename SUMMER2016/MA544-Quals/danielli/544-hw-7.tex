...
\end{problem}
\begin{solution}
  For a fixed $x_0\in\bbR^n$ we must show that for every sequence $x\to
  x_0$, we have
  \[
    \liminf_{x\to x_0} g(x)\geq g(x_0).
  \]
  By the definition of the limit supremum, $\liminf_{x\to x_0} g(x)$ is the
  limit over the set of infimums $\inf_{x\in\{x_n\},n\geq N} g(x)$
\end{solution}
