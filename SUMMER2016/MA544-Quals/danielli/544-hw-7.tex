...
\begin{problem}[Wheeden \& Zygmund Ch.\@ 4, Ex.\@ 9]
  ...
  \item Show that the limit of a decreasing (increasing) sequence of
    functions u.s.c.\@ (l.s.c.) at \(x_0\) is u.s.c.\@ (l.s.c.) at
    \(x_0\). In particular, the limit of a decreasing (increasing) sequence
    of functions continuous at \(x_0\) is u.s.c.\@ (l.s.c.) at \(x_0\).
  \item Let \(f\) be u.s.c.\@ and less than \(\infty\) on \([a,b]\). Show
    that there exists continuous \(f_k\) on \([a,b]\) such that
    \(f_k\downarrow f\).
  \end{enumerate}
\end{problem}
\begin{solution}
