...
\begin{problem}[Wheeden \& Zygmund Ch.\@ 4, Ex.\@ 9]
  ...
  \item Show that the limit of a decreasing (increasing) sequence of
    functions usc (lsc) at $x_0$ is usc (lsc) at $x_0$. In particular, the
    limit of a decreasing (increasing) sequence of functions continuous at
    $x_0$ is usc (lsc) at $x_0$.
...
\begin{problem}[Wheeden \& Zygmund Ch.\@ 4, Ex.\@ 11]
  Let $f$ be defined on $\bfR^n$ and let $B(x)$ denote the open ball
  $\left\{\,y:|x-y|<r\,\right\}$ with center $x$ and fixed radius $r$. Show
  that the function $g(x)=\sup\left\{\,f(y):y\in B(x)\,\right\}$ is lsc and
  the function $h(x)=\inf\left\{\,f(y):y\in B(x)\,\right\}$ is usc on
  $\bfR^n$. Is the same true for the closed ball
