...
For the last part consider the map
\begin{equation}
  \label{eq:non-usc-map}
f(x)\coloneqq
\begin{cases}
1&\text{if $x<x_0$}\\
0&\text{if $x\geq x_0$}
\end{cases}
\end{equation}
defined earlier in the section. Let
$h(x)\coloneqq\inf\bigl\{\,f(y)\bigm|y\in\overline{B(x)}\,\bigr\}$. Let
$x$ be a distance of $r$ to the right of $x_0$. Then $1/2>h(x)=0$. However,
for any $\delta>0$, $y\in B_\delta(x)$ we have $x'\in\overline{B(y)}$ such
that $x'<x_0$. Hence, $h(y)=1>1/2$ so $h$ is not usc.
\end{proof}
\newpage
