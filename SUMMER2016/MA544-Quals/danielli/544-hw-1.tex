...
\end{problem}
\begin{proof}
  Recall that, given a partition $\Gamma$ of the interval$[a,b]$,
  $P_\Gamma$ and $N_\Gamma$ are defined to be the sum of the positive and,
  respectively, the negative terms of $S_\Gamma$ and that the positive
  variation $P$ and negative variation $N$ are
  \[
    P\coloneq\sup_\Gamma P_\Gamma \qquad N\coloneq\sup_\Gamma N_\Gamma.
  \]

  Now, we aim to show that if $[a',b']\subset[a,b]$, then $P[a',b']\leq
  P[a,b]$ and $N[a',b']\leq N[a,b]$. We can do this in two ways.

  First, by Theorem 2.6 we have
  \[
    P=\tfrac{1}{2}(V+f(b)-f(a))\quad\text{and}\quad N=\tfrac{1}{2}(V-f(b)+f(a)).
  \]
  Moreover, Theorem 2.2 (i) tells us that $V[a',b']\leq V[a,b]$ so
  \[
    P[a',b']=\tfrac{1}{2}(V[a',b']+f(b')-f(a'))
  \]
\end{proof}
