...
\begin{solution}
  ...
  \begin{align*}
    \int_E f\diff x&=1
  \intertext{while}
    \lim_{n\to\infty}\int_E f\diff x&=\infty.
  \end{align*}
\end{solution}
...
\end{problem}
\begin{solution}
  \(\implies\) Suppose that \(f\) is integrable. Then, by definition,
  \[
    \int_E f\diff x=\sup\left\{\,\int_E s\diff x:\text{\(s\) is a simple
        function and \(s\leq f\)}\,\right\}<\infty.
  \]
  Thus,
\end{solution}
