...
$f$. For $r>0$, let
\[
A(r)\coloneq\frac{1}{|r|^n}\int_{B(0,r)}|f(\bfx-\bfy)-f(\bfx)|\diff\bfy.
\]
Show that:
...
\]
Set $F(\bfy)\coloneq |f(\bfx-\bfy)-f(\bfx)|$ and consider said quantity
\begin{align*}
|A(s)-A(r)|
&=\left|
...
\begin{problem}
Let $f\in L(\bbR)$, and let $F(t)\coloneq\int_{\bbR}f(x)\cos(tx)dx$.
\begin{enumerate}[label=(\alph*),noitemsep]
