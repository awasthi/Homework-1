...
% Following the style of \cite[Ch.\@ 1, p.\@ 2]{wheeden-zygmund},
  ...
  % in $V_{k+1}$ so $V_{k+1}\supseteq V_k$. Hence, we have
  % $V_k\nearrow\liminf E_k$.
...
\begin{solution}
  ...
  \]
  % \begin{remarks*}
  %   The problem can be solved using more elementary results, but the setup
  %   is a bit longer and amounts to proving a special case of Fatou's lemma
...
\begin{solution}
  ...
  $F(x)=|r|^nm(B(\mathbf{0},1))$ and it is easy to see that $F$ is both
  monotonically increasing and continuous since it is a polynomial in $r$.
\end{solution}
