...
\begin{problem}
  ...
  $rE=\left\{\,r x : x \in E\,\right\}$. Prove that $rE$ is measurable,
  and that $|rE|=|r|^n|E|$.
\end{problem}
\begin{solution}
  Define a map $T\colon\bbR^n\to\bbR^n$ by $T x = r x $. Note that $T$ is
  \href{https://en.wikipedia.org/wiki/Lipschitz_continuity}{\emph{Lipschitz
      continuous}} since for any $ x , y \in\bbR^n$, the equality
  \begin{equation}
    \label{eq:prep:1:1}
    |T x -T y |=|r x -r y |=|r|| x - y |
  \end{equation}
