...
\end{problem}
\begin{solution}
  % First, we show that if \(f\colon[a,b]\to\bbR\) is absolutely continuous
  % on \([a,b]\), it is continuous on \([a,b]\). Let \(\varepsilon>0\) be
  % given. Then, since \(f\) is absolutely continuous on \([a,b]\), there
  % exists a \(\delta>0\) such that that for every finite disjoint collection
  % of open intervals \(\{(a_n,b_n)\}\), \(1\leq n\leq N\), satisfying
  % \(\sum_{n=1}^Nb_n-a_n<\delta\), we have
  % \[
  %   \sum_{n=1}^N\bigl|f(b_n)-f(a_n)\bigr|<\varepsilon.
  % \]
  % In particular, for any \(x\in[a,b]\), with \(|x-y|<\delta\),
  % \(\{(x,y)\}\) (if \(|a-x|<\delta\) or if \(|b-x|<\delta\), consider
  % instead \((y,x)\)) is a disjoint collection of open intervals with
  % \(y-x<\delta\). Thus,
  % \[
  %   \bigl|f(x)-f(y)\bigr|<\varepsilon.
  % \]
  % Thus, \(f\) is continuous on \([a,b]\); the argument can be easily
  % adapted to the case when \(x=a,b\).

  % Since \(f\) is continuous, it suffices to show that
  % \(\lim_{\Delta(\Gamma)\to 0} S_\Gamma<\infty\); where \(\Delta(\Gamma)\)
  % denotes the partition size of \(\Gamma\), i.e., the
  % \(\max\left\{\,x_i-x_{i-1}:1\leq i\leq\card \Gamma\,\right\}\).
