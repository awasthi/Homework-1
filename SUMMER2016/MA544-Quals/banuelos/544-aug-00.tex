...
\begin{solution}
  ...
  $|F(x)-F(x')|<\varepsilon$. Then, since $f_n\to f$ in measure, there
  exist an index there exists an index $N$ such that $n\geq N$ implies
  \[
    \mu\left(%
      \left\{\,x\in X:|f(x)-f_n(x)|\geq\delta\,\right\}%
    \right)%
    <\varepsilon.
  \]
  We want to show that the above (or some variation thereof) implies that
  the measure of the set
  \[
    \left\{\,x\in X:|F(f(x))-F(f_n(x))|\geq\delta\,\right\}%
  \]
  approaches $0$ as $n$ approaches $\infty$.
\end{solution}
