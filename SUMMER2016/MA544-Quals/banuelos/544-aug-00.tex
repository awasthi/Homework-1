...
\begin{solution}
  ...
  % of $E_n$. If we can show that $f\in L^p(X)$ we win as it is known that if
  % $\mu(X)<\infty$ then $L^r(X)\subset L^p(X)$ for all $1\leq r<p$. Our goal
  % is therefore to bound the integral
  % \[
  %   \int_X f^p\diff\mu.
  % \]
  ...
  \]
  i.e., the distribution function of $f$.
\end{solution}
