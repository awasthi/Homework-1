...
\begin{solution}
  ...
  \]
  Note that the $E_n$ are pairwise disjoint and their union is the set
  $X$. Thus, the Borel--Cantelli M\textru{Мы русские}

  We aim to show that $f\in L^r(X)$ for $1\leq r<p$, i.e., that
  \[
    \int_X f^r\diff x<\infty.
  \]

  Now, since the measure of $X$ is finite and, by properties of the
  Lebesgue integral, we may safely ignore the set $E\subset X$ where
  $f(x)<1$ for every $x\in E$ since, by properties of the Lebesgue
  integral,
  \[
    \int_X f^r\diff x=\int_X
  \]
\end{solution}
