...
\end{problem}
\begin{solution}
  Define $g_n,h_n\colon\calF\to[0,\infty]$ for $n\in\bfN$ by
  \begin{align*}
    g_n(\lambda)%
...
\begin{problem}
  ...
  \item Let $(X,\calF,\mu)$ be a finite measure space. Let
    $F\colon\bfR\to\bfR$ be continuous and $f_n\to f$ in measure. Prove
...
\begin{solution}
  ...
  $\varepsilon>0$ and $\delta>0$ there exists $N\in\bfN$ such that
  $n\geq N$ implies
  \[
    \mu\left(\left\{\,%
  ...
  \]
  for almost every $x\in X$, there exists $N_x\in\bfN$ such that $x\notin
...
\begin{solution}
  ...
  $n\in\bfN$. Thus, by Hölder's inequality
  \[
    \begin{aligned}
...
\begin{problem}
  Let $f\in L^1(\bfR)$. Consider the function
  \[
    F(x)=\int_{\bfR} \exp(\rmi xt) f(t)\diff t.
  \]
  \begin{enumerate}[label=(\roman*),noitemsep]
  \item Show that $F\in L^\infty(\bfR)$ and that $F$ is continuous at every
    $x\in\bfR$. Moreover, if $|t|^kf(t)\in L^\infty(\bfR)$ for all $k\geq
    1$, show that $F$ is infinitely differentiable, i.e., $F\in
    C^\infty(\bfR)$.
  \item Suppose $f$ is continuous as well as in $L^1(\bfR)$. Show that
    $\lim_{|x|\to\infty} F(x)=0$.
  \end{enumerate}
  [\emph{Hint}: Using $\exp(-\rmi\pi)=-1$, write
  $F(x)=\left(\int_{\bfR}(\exp(\rmi xt)-\exp(\rmi xt-\rmi\pi))\right)/2$.]
\end{problem}
\begin{solution}
  ...
  \[
    |F(x)|\leq\|f\|_1.
  \]
  Since $|\blank|\colon\bbC\to\bfR$ is convex, by Jensen's inequality, we
  have
  \begin{align*}
    |F(x)|
    ...
    &=\|f\|_1.
  \end{align*}
  Thus, $\esssup_{x\in\bfR}F\leq\|f_1\|$ so $F\in L^\infty(\bfR)$. To see
  ...
  \[
    \int_\bfR |f|\diff t-\int_\bfR|f_n|\diff t=%
