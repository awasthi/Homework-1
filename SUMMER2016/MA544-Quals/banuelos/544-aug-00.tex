...
\begin{solution}
  ...
  \begin{align*}
    ...
    &\leq\esssup_{t\in\bbR}|{\exp(\rmi xt)}|\cdot\|f\|_1\\
    &=\esssup_{t\in\bbR}\left(\sqrt{\cos^2(xt)+\sin^2(xt)}\right)\|f\|_1\\
    &=\|f\|_1.
  \end{align*}
  Thus, $\esssup_{x\in\bbR}F\leq\|f_1\|$ so $F\in L^\infty(\bbR)$. To see
  that $F$ is continuous, take let $\varepsilon>0$ be given. Then, we want
  to show that for some $\delta>0$, $|x-y|<\delta$ implies
  \begin{align*}
    |F(x)-F(y)|&=%
                 \left|\int_\bbR\exp(\rmi xt)f(t)\diff t
                 -\int_\bbR\exp(\rmi yt)f(t)\diff t
                 \right|\\
               &=\left|\int_\bbR\exp(\rmi xt)f(t)
                 (1-\exp(\rmi (y-x)t))\diff t
                 \right|\\
               &<%
                 \varepsilon
  \end{align*}
