...
\begin{solution}
  ...
  For (ii) since $F$ is continuous and the $f_n,f$ are measurable,
  $F(f_n),F(f)$ are measurable. Suppose that $f_n\to f$ in measure. Then
  given $\varepsilon,\delta>0$, there exists an index $N$ such that $n\geq
  N$ implies
  \[
    \mu\left(%
      \left\{\,x\in X:|f(x)-f_n(x)|\geq\varepsilon\,\right\}%
