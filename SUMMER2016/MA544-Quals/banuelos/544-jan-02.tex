...
\begin{solution}
  ...
  $\overline{A+B}\neq A+B$. Let $x\in\bbR$ then given $\varepsilon>0$, and
  consider $x-m-\varepsilon<x-m+\varepsilon$ (where we can choose $m$ to
  be, e.g., $m=\lfloor x \rfloor$). By the density of $\bbQ$ in $\bbR$,
  there exists a rational number $a/b\in\bbQ$ such that
  \[
    \frac{x-m-\varepsilon}{\sqrt{2}b}<\frac{a}{b}<\frac{x-m+\varepsilon}{\sqrt{2}b}
  \]
\end{solution}
...
\begin{solution}
  ...
  there exists $\delta>0$ such that $|x-y|<\delta$ implies
  \[
    |f'(x)-f'(y)|<\varepsilon.
  \]
  Now,
