...
\begin{problem}
  ...
  $\langle a\rangle\cap\langle b\rangle=\{e\}$, then $\ord(ab)=mn/(m,n)$.
\end{problem}
\begin{solution}
...
\begin{solution}
  ...
  \begin{subproof}[Proof of claim]
    Since $\card H=6$, $[A_4:H]=2$ which implies that $H$ is must be a
    normal subgroup of $A_4$. Now, consider the collection of $G/H$ of
    right-cosets of $H$ in $G$. By Theorem 5.4, $G/H$ is a group with order
    $\card(G/H)=2$ so either $\bar\sigma=\bar e$ or
