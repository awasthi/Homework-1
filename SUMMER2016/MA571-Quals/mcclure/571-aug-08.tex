...
\begin{solution}
  ...
  \[
    W=\bigcap_{i=1}^N\pi_1(U_i\times V_i)=\bigcap_{i=1}^NU_i
  \]
...
\end{problem}
\begin{solution}
  % Consider the punctured plane \(X=\bbR^2\setminus\{(0,0)\}\) and fix the
  % point \(x_0=(1,0)\). Then since \(X\simeq S^1\), the fundamental group of
  % \(X\) is isomorphic to \(\bbZ\). Let \(f\colon X\to X^+\) be the map
  % given by \(f(x,y)=(x^2,y)\) where
  % \(X^+=\bigl\{\,(x,y)\in X:x>0\,\bigr\}\). This map is onto because for
  % any \((x,y)\in X^+\), \(\bigl(\sqrt{x},y\bigr)\mapsto (x,y)\). However,
  % the set \(X^+\) is homeomorphic to the plane \(\bbR^2\) (you may, for
  % example, take the map \((x,y)\mapsto (\log x,y)\) this map is continuous
  % with a continuous inverse \((x,y)\mapsto (\exp x,y)\)) which is
  % path-connected and therefore has fundamental group isomorphic to
  % \(\{0\}\).
