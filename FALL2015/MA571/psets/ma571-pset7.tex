...
is continuous.
% \begin{align*}
% \pi_1(G_f\cap X\times(Y-V))
% &=\left\{\,x\;\middle|\;\text{$\pi(x',y)=x$ for at least one
%   $(x',y)\in G_f\cap X\times(Y-V)$}\,\right\}\\
% &=\left\{\,x\;\middle|\;\text{$\pi(x',f(x'))=x$ for at least one
%   $(x,f(x'))\in X\times(Y-V)$}\,\right\}\\
% &=\left\{\,x\;\middle|\;f(x)\in Y-V \right\}\\
% &=f^{-1}(Y-V)
% \end{align*}
\end{proof}
\newpage
