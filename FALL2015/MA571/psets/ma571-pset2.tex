...
$Y$, respectively. Then, $(X\setminus A)\times Y$ and $X\times
(Y\setminus A)$ are basic open sets in $X\times Y$. Thus
\[
(X\setminus A)\times (Y\setminus B)
=
\left((X\setminus A)\times Y\right)
\cap
\left(X\times(Y\setminus B)\right)
\]
\end{proof}
\newpage
