...
\end{problem}
\begin{proof}
% Let $\mathfrak{a}$ be an ideal of $R$. Then $\mathfrak{a}$ is
% contained in a maximal ideal, say $\mathfrak{m}$. Then
% $\mathfrak{m}=\langle a\rangle$, i.e, $\mathfrak{m}$ is
% principal. By the previous problem, we have that $R\cong
% R_1\times\cdots\times R_n$ where each $R_i$ is a local Artinian
% ring. Then $\mathfrak{a}\cong\prod\mathfrak{a}_i$ for
% $\mathfrak{a}_i\subset R_i$.
