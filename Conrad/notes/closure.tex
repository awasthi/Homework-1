...
\[
  A^\circ=\Int A=\bigl\{\,a\in A:\text{there exists \(\varepsilon>0\) such that
    \(B(a,\varepsilon)\subseteq A\)}\,\bigr\}
\]
...
\begin{example}
  ...
  \(-1<y<1\) then for
  \[
    r=\min\bigl\{|-1-x|,|1-x|,1-1-y|,|1-y|\bigr\}>0
  \]
  ...
  a limit of a sequence in \(A\). We just have to show
\end{example}
