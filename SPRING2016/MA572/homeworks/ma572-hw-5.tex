...
\end{problem}
\begin{proof}
Let $f\colon\bfR^n\to\bfR^n$ be an invertible linear map. Then $\det f\neq
0$. Following the hint, we construct a chain homotopy $P\colon
C_i(\bfR^n)\to C_i(\bfR^n)$ such that $\partial\circ
P+P\circ\partial=f_\#-g_\#$ where $g_\#$ is the chain map induced by the
linear map $g\colon\bfR^n\to\bfR^n$ whose matrix representation is
upper-triangular with $\pm 1$'s in the diagonal. Now
\end{proof}
\newpage
