...
\end{problem}
\begin{proof}
By the commentary at the end of the proof of Theorem 8.8 (and this
\texttt{StackExchange} post
\url{http://math.stackexchange.com/questions/37832/on-the-hölder-inequality})
we must prove the following:
\begin{quote}
If $1\leq p\leq\infty$, then $f\in L^p(E)$ if $fg\in L(E)$ for every $g\in
L^{p'}(E)$, with $1/p+1/p'=1$.
\end{quote}

We prove this for $p=1$ first. Recall that by convention, if $p=1$ its
conjugate exponent, $p'$, is $\infty$ and vice versa. Therefore, suppose
\begin{equation}
  \label{eq:1:hypothesis}
\|fg\|_1\leq\|f\|_1\cdot\|g\|_{\infty}
\end{equation}
for every $g\in L^\infty(M)$. By the definition of the essential supremum,
$|g|$ is bounded almost everywhere in $E$ by $\|g\|_\infty$. Then, the
result follows by Theorem 8.8 since
\[
\|f\|_1=\sup_E fg
\]
which, by Equation \eqref{eq:1:hypothesis}, gives us
\[
\sup \int_E fg\leq \sup \left(\|f\|_1\cdot\|g\|_\infty\right)
\]
\end{proof}
\newpage
