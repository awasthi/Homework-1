...
x\longrightarrow 0.
\]
Observe that the function $\Phi\colon\bfR_{>0}\to\bfR$ given by
$\Phi(x)\coloneqq x/(1+x)$ is increasing on $\bfR_{>0}$ and $0<\Psi(x)<1$,
hence
\[
\begin{aligned}
  \int_{\left\{\,x\in
...
\begin{problem}
Define the \emph{gamma function} $\Gamma\colon\bfR_{>0}\to\bfR$ by
\[
\Gamma(y)\coloneqq\int_0^\infty e^{-u}u^{y-1}\diff u,
\]
and the \emph{beta function} $\beta\colon\bfR_{>0}\times\bfR_{>0}\to\bfR$
by
\[
...
the function $u\mapsto e^{-u}u^{y-1}$ is in $L(\bfR_{>0})$ for all
$y\in\bfR_{>0}$.
\item Show that
\[
...
\end{problem}
\begin{proof}
(a) Fix $y\in\bfR_{>0}$. Then we must show that $\Gamma(y)<\infty$. First,
