...
$\lambda_{\bbR^n}(E)<\infty$. Prove that a nonnegative function $f$ defined
on $E$ is integrable if and only if
\[
\sum_{k=0}^\infty\lambda_{\bbR^n}\left(\left\{\,\bfx\in E:f(\bfx)\geq
    k\,\right\}\right)<\infty.
\]
\end{problem}
\begin{proof}
...
\lambda_{\bbR^n}(E)=\sum_{k\in\bbN}\lambda_{\bbR^n}(E_k)<\infty.
\end{equation}
and
\begin{equation}
\label{eq:monotonicity-of-lebesgue-3}
\lambda_{\bbR^n}(F_k)=\sum_{\ell=k}^\infty\lambda_{\bbR^n}(E_k).
\end{equation}
In particular, by Chebyshev's inequality, we have
\begin{equation}
\label{eq:chebyshevs-3}
\int_Ef(\bfx)\diff\bfx\geq
k\lambda_{\bbR^n}(F_k)=k\sum_{\ell=k}^\infty\lambda_{\bbR^n}(E_k)
\end{equation}
$\impliedby$:
\end{proof}
...
$\lambda_{\bbR^n}(E)<\infty$. If $f$ and $g$ are measurable functions on
$E$, define
\[
