...
\[
\int_{\bbR^n\minus B(0,\varepsilon)}f(x)\;dx\leq\frac{C}{\varepsilon}.
\]
\end{problem}
\begin{proof}
\end{proof}
...
\[
\sum_{k=0}^\infty\lambda\left(\left\{\,\bfx\in E\;\middle|\;f(\bfx)\geq
    k\,\right\}\right)<\infty.
\]
\end{problem}
\begin{proof}
...
\[
\Gamma(y)\coloneqq\int_0^\infty e^{-u}u^{y-1}\;du,
\]
and the \emph{beta function} $\beta\colon[0,\infty)\times[0,\infty)\to\bbR$
by
\[
\beta(x,y)\coloneqq\int_0^1 t^{x-1}(1-t)^{y-1}\;dt.
\]
