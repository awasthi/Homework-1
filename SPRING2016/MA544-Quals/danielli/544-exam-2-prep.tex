...
\int_{E_k}f(\bfx)\diff\bfx\geq k|E_k|.
\end{equation}
But $f(\bfx)\leq (k+1)\chi_{E_k}(\bfx)$ on $E_k$ so
\begin{equation}
\label{eq:upper-estimate-3}
\int_{E_k}f(\bfx)\diff\bfx\leq \int_{E_k}(k+1)\mathbf{1}\diff\bfx=(k+1)|E_k|.
\end{equation}
