...
\end{problem}
\begin{proof}
Recall that $f\in L^1(\bfR^n)$ if and only if $|f|\in L^1(\bfR^n)$. Since
$|f|$ is nonnegative, $|f|\in L^1(\bfR^n)$ implies that $|f|$ is finite
a.e.\@ on $\bfR^n$. Pick some sufficiently large real number $M$ such that
$M>|f|$ a.e.\@ on $\bfR^n$.
\end{proof}
