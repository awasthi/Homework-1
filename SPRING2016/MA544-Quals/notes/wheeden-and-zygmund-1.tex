...
\limsup E_k\coloneqq\bigcap_{j=1}^\infty\bigcup_{k=j}^\infty E_k,\qquad
\liminf E_k\coloneqq\bigcup_{j=1}^\infty\bigcap_{k=j}^\infty E_k,
\end{equation}
...
\emph{boundary} of $E$, we mean $\partial E\coloneqq\bar E\setminus
E^\circ$.
\begin{theorem}[1.5]
...
$E$. Let $B'(\bfx_0,\delta)= B(\bfx_0,\delta)\setminus\{\bfx_0\}$
...
\item $M=\limsup_{\bfx\to\bfx_0;\bfx\in E} f(\bfx)$ if and only if
  \textnormal{(i)} there exist ${\bfx_k}$ in $E\setminus\{\bfx_0\}$ such that
...
\[
\begin{aligned}
P_\Gamma&=P_\Gamma[f;a,b]\coloneqq\sum_{i=1}^m{[f(x_i)-f(x_{i-1})]}^+\\
N_\Gamma&=N_\Gamma[f;a,b]\coloneqq\sum_{i=1}^m{[f(x_i)-f(x_{i-1})]}^-
\end{aligned}
\]
