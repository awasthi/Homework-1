...
\begin{solution*}
  ...
  \begin{solution*}[Proof]
    ...
    \begin{align*}
      ...
      &=\left(\frac{\lambda}{\lambda-t}\right)^\alpha\\
      &=\left(1-\frac{t}{\lambda}\right)^{-\alpha}.\qedhere
    \end{align*}
  \end{solution*}
  With this handy lemma in our hands we can quickly show that \(X+Y\sim
  G(\alpha+\beta,\lambda)\) as we now do. First, since \(X\) and \(Y\) are
  ...
  variable; i.e., \(X+Y\sim G(\alpha+\beta,\lambda)\).
\end{solution*}
