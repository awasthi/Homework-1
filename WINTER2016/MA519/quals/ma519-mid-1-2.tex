...
\end{problem}
\begin{solution*}
  Write \(S\) as the sum \(\sum_{k=1}^r X_k\) where
  \[
    X_k
    =
    \begin{cases}
  ...
  \begin{equation}
    ...
    \end{aligned}
  \end{equation}
  and this is as far as the definition of expected value will take us
  unless we make an important observation about the \(E(X_k)\).

  In fact, We claim that \(E(X_k)=E(X_j)\) for all \(1\leq k,j\leq r\). To
  see this, let us unwind the definition of expected value as we now do
  \[
    E(X_k)=0\cdot P(X_k=0)+1\cdot P(X_k=1)=P(X_k=1).
  \]
  Similarly we have \(E(X_j)=P(X_j=1)\). But
  \[
    P(X_k)=\frac{(r-1)^n}{r^n}=P(X_j).
  \]
  Thus, \(E(X_k)=E(X_j)\).

  Consequently, \eqref{eq:yip:mid-1:1-expanded} becomes
  \[
    E(S)=rE(X_k)=r\frac{(r-1)^n}{r^n}=\frac{(r-1)^n}{r^{n-1}}.
  \]

  Regarding the variance, we first need to compute the second moment of
  \(S\) as we now do
  \begin{align*}
    E(S^2)
  ...
  \end{align*}
  Let us now take care of that second term in the expansion of \(E(S^2)\);
  i.e., the sum \(\sum_{j\neq k}E(X_jX_k)\). Taking this sum and expanding
  it, we have
  \[
    \sum_{j\neq k}E(X_jX_k)=%
    \sum_{j\neq k}P(X_j=1,X_k=1)=%
  ...
  \[
    \Var(S)=%
    E(S^2)-E(S)^2=%
    \left[\frac{(r-1)^n}{r^{n-1}}+%
    \binom{r}{2}\frac{(r-2)^n}{r^n}\right]-%
