...
\end{problem}
\begin{solution*}
  Every path in the figure in the problem prompt is a possible path from
  \(A\) to \(B\). We can count them, in which case, we have \(12\) distinct
  paths or we can use the basic principle of counting to arrive at the
  desired quantity \(4\cdot 3=12\).
\end{solution*}
...
\end{problem}
\begin{solution*}
  This problem is slightly more complicated than its predecessor, but we
  win if we can count all of the possible paths from \(A\) to \(B\) passing
  through that circled point. We know a priori that the number of paths is
  less than or equal to \(12\). If we do count these, we arrive at the
  quantity \(6\) which is less than or equal to \(12\).
\end{solution*}
