...
\hline
\text{Name}&\text{Symbol}&\text{mass/density function}&\text{mean}&\text{variance}&\text{MGF}\\
\hline
\text{Bernoulli}
&\Ber(p)%
...
\text{beta}
&B(\alpha,\beta)
&\text{\(\frac{x^{\alpha-1}(1-x)^{\beta-1}}{B(\alpha,\beta)}\)
  for \(0\leq x\leq 1\)}
...
\begin{solution*}
  ...
  \begin{align*}
    p_1(100,10,10)&\approx 0.3487,\\
    p_2(100,10,10)&\approx 0.3305.\qedhere
  \end{align*}
\end{solution*}
...
\begin{solution*}
  ...
  For \(n=100\), we have \(E(X)\approx 36.6032\).
\end{solution*}
