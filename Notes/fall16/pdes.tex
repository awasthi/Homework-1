...
\begin{definition}
  ...
  \[
    \bfF\colon\bfR^{mn^k}\times\bfR^{mn^{k-1}}\times\dotsc\times\bfR^{mn}\times\bfR^m\times
    U\too\bfR^m
  \]
  is given and
  \[
...
\(x=(x_1,\dotsc,x_n)\in\bfR^n\) is a point in space, \(t\geq 0\) denotes a
typical time and \(u\colon\bfR\times[0,\infty)\to\bfR\) is the unknown,
\(u=u(x,t)\). We write \(Du=D_xu=(u_{x_1},\dotsc,u_{x_n})\) for the
gradient of \(u\) with respect to the spatial variable \(x\).
